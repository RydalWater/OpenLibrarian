...
from utils.Session import (
    async_logged_in,
    async_get_session_info,
    async_set_session_info,
    cache_get,
    cache_set,
    cache_key,
    cache_delete,
...
async def user_profile(request):
    ...
    # Otherwise return the user profile
        ...
        if (
            request.method == "POST"
            and await async_logged_in(request)
            and request.POST.get("save")
        ):
            ...
            await async_set_session_info(
                request, profile=session["profile"], nym=nym_field
            )
            session = await async_get_session_info(request)
        ...
        if (
            request.method == "POST"
            and await async_logged_in(request)
            and "refresh" in request.POST
        ):
            npub = session["npub"]
            if session["relays"] is None:
                profile, relays, added_relays = await fetch_profile_info(npub=npub)
            else:
                profile, relays, added_relays = await fetch_profile_info(
                    npub=npub, relays=session["relays"]
                )
            nym = profile.get("nym")
...
async def user_relays(request):
    ...
    # Otherwise return the user relays
        ...
        if (
            temp_session["mod_relays"] in [None, {}]
            or "mod_relays" not in temp_session.keys()
            or request.POST.get("cancel")
        ):
        ...
        if request.method == "POST":
            ...
            if (
                request.POST.get("add_relay")
                and request.POST.get("add_relay_url")
                and request.POST.get("relay_option")
            ):
                if request.POST.get("relay_option") == "R":
                    temp_session["mod_relays"][request.POST.get("add_relay_url")] = (
                        "READ"
                    )
            ...
            # Remove_relay button modfies the content of the mod_real updates session information
                ...
                if (
                    read_count == 1
                    and temp_session["mod_relays"][request.POST.get("remove")]
                    in ("READ", None)
                    or write_count == 1
                    and temp_session["mod_relays"][request.POST.get("remove")]
                    in ("WRITE", None)
                    ...
                    session["mod_relays"] = temp_session["mod_relays"].copy()
                    await async_set_session_info(
                        request, mod_relays=session["mod_relays"]
                    )

            # Save changes submits event to write relays.
            elif request.POST.get("save"):
                if session["nsec"] and session["nsec"].upper() == "Y":
                    update, builder, new_relays_dict = await edit_relay_list(
                        session["relays"], temp_session["mod_relays"]
                    )
                    if update:
                        events = nostr_prepare([builder])
    ...
    if not await async_logged_in(request):
        ...
        if request.method == "GET":
            ...
            if connctions is None:
                friends = await fetch_social_list(
                    relays=session["relays"], npub=session["npub"], list_type="follow"
                )
                muted = await fetch_social_list(
                    relays=session["relays"], npub=session["npub"], list_type="mute"
                )

                # Cache the friends list for 30 minutes
                await cache_set(
                    await cache_key(key_str, session),
                    {"friends": friends, "muted": muted},
                    1800,
                )
            else:
        ...
        # POST requests
            ...
            if request.POST.get("follow_user"):
                noted, build = await add_follow(
                    session["relays"],
                    npub=session["npub"],
                    follow_id=request.POST.get("follow_user"),
                )
                ...
                noted, build = await remove_follow(
                    session["relays"],
                    npub=session["npub"],
                    follow_id=request.POST.get("remove"),
                )
                if build is not None:
                    events = nostr_prepare([build])
            event_relays = get_event_relays(relays_dict=session["relays"])
