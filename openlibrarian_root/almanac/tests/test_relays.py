...
class RelaysUnitTestCase(TestCase):
    ...
    def test_post_save_invalid(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertNotEqual(
            response.context["session"]["relays"],
            response.context["session"]["mod_relays"],
        )
    ...
    def test_post_cancel(self):
        ...
        self.assertEqual(
            response.context["session"]["mod_relays"], {"wss://relay.damus.io": None}
        )
        response = self.client.post(
            self.url,
            {
        ...
        self.assertEqual(
            response.context["session"]["mod_relays"],
            {"wss://relay.damus.io": None, "wss://nostr.mom": None},
        )
        self.assertEqual(
            response.context["session"]["relays"], {"wss://relay.damus.io": None}
        )
