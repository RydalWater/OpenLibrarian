...
class ProfileUnitTestCase(SettingsUnitTestCase):
    ...
    def test_post_save_displayname(self):
        ...
        response = self.client.post(
            self.url, {"save": "Save", "edit_displayname": "new_display_name"}
        )
        sys.stdout = sys.__stdout__
        # output = capturedOutput.getvalue().strip()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["session"]["profile"]["displayname"], "new_display_name"
        )
    ...
    def test_post_save_about(self):
        ...
        sys.stdout = capturedOutput
        response = self.client.post(
            self.url, {"save": "Save", "edit_about": "new_about"}
        )
        sys.stdout = sys.__stdout__
        # output = capturedOutput.getvalue().strip()
    ...
    def test_post_save_picture(self):
        ...
        response = self.client.post(
            ...
            {
                "save": "Save",
                "edit_picture": "https://pfp.nostr.build/9d45acc985222226824a1e91850bf3d450b8b5964ef14f41fbd4df2c9dcc3241.jpg",
            },
        )
        sys.stdout = sys.__stdout__
        # output = capturedOutput.getvalue().strip()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["session"]["profile"]["picture"],
    ...
    def test_post_save_nip05(self):
        ...
        sys.stdout = capturedOutput
        response = self.client.post(
            self.url, {"save": "Save", "edit_nip05": "Rydal@gitlurker.info"}
        )
        sys.stdout = sys.__stdout__
        # output = capturedOutput.getvalue().strip()
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.context["session"]["profile"]["nip05"], "Rydal@gitlurker.info"
        )
