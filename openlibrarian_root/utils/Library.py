        ...
        # Existing library (Event)
            ...
            if type(kwargs["event"]) is not Event:
                ...
                if "title" not in tags_dict.keys():
                    ...
                    section = list(section_title_map.keys())[
                        list(section_title_map.values()).index(tags_dict["title"])
                    ]
        ...
        for book in self.books:
            ...
            tag = Tag.custom(
                TagKind.SINGLE_LETTER(
                    single_letter=SingleLetterTag.lowercase(Alphabet.I)
                ),
                [f"isbn:{clean_isbn}"],
            )
            if book_obj["h"] == "Y":
...
async def fetch_libraries(npub: str = None, relays: dict = None):
    ...
    if npub in [None, ""] or check_npub(npub) is False:
        ...
        fetched = await nostr_get(
            wait=10, filters={"libraries": filter}, relays_dict=relays
        )
...
async def prepare_libraries(
    libEvents: list = None, npub: str = None, read_only: bool = False
):
    """Build libraries from events"""
    # Check list of events have been provided
    if libEvents is None:
        ...
        unique_events = remove_dups_on_id(events, "library")
