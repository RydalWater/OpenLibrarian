...
async def login_npub_view(request):
    ...
    if request.method == "POST":
        ...
        if valid_npub:
            ...
            tasks = [
                fetch_libraries(npub=npub, relays=relays),
                fetch_interests(npub, relays),
            ]
            raw_libraries, interests = await asyncio.gather(*tasks)
            libraries = await prepare_libraries(
                libEvents=raw_libraries, npub=npub, read_only=True
            )
            nym = profile.get("nym")
                ...
                if library["s"] in ("CR", "HR"):
                    for book in library["b"]:
                        if "Hidden" not in book["i"]:
                            isbns.append(book["i"])
            tasks_prog_review = [
                fetch_progress(npub=npub, isbns=isbns, relays=relays),
                fetch_reviews(npub=npub, relays=relays, isbns=isbns),
            ]
            ...
            context = {"form": form, "noted": "false:Invalid NPUB."}
            return render(request, "circulation_desk/login_npub.html", context)
...
async def login_nsec_view(request):
    return await login_rw_view(
        request, NsecForm, "circulation_desk/login_nsec.html", "/login/nsec"
    )


async def login_seed_view(request):
    return await login_rw_view(
        request, SeedForm, "circulation_desk/login_seed.html", "/login/seed"
    )
...
async def login_nip07_view(request):
    ...
    if request.method == "GET":
        ...
        return await login_rw_view(
            request, None, "circulation_desk/login_nip07.html", "/login/nip07"
        )
