...
class LoginNsecFunctionalTestCase(BaseFunctionalTest):
    ...
    def test_invalid_nsec(self):
        ...
        self.driver.find_element(by=By.ID, value="nsec").send_keys(
            "nsecmadeup123456blahblah"
        )
...
class LoginNsecUnitTestCase(BaseUnitTests):
    ...
    def test_login_session_data(self):
        ...
        client.get("/login-nsec/")
        data = {"npubValue": TC_NPUB, "hasNsec": "Y", "decryptedEvents": []}
        client.post("/login-nsec/", data, content_type="application/json")
    ...
    def test_get_request(self):
        ...
        client = Client()
        response = client.get(reverse("circulation_desk:fetch_events"))
    ...
    def test_post_request_empty_data(self):
        ...
        with self.assertRaises(Exception) as e:
            self.client.post(
                reverse("circulation_desk:fetch_events"),
                data=json.dumps({}),
    ...
    def test_post_request_badNpub_data(self):
        ...
        with self.assertRaises(Exception) as e:
            self.client.post(
                reverse("circulation_desk:fetch_events"),
                data=json.dumps({"npubValue": "npub1234567"}),
                content_type="application/json",
            )
    ...
    def test_post_request_invalid_credentials(self):
        ...
        response = self.client.post(
            reverse("circulation_desk:login-nsec"),
            data=json.dumps(data),
            content_type="application/json",
        )
