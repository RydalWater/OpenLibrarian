...
class ShelvesUnitTestCase(TestCase):
    ...
    def test_library_read_only(self):
        ...
        session = self.client.session
        session["npub"] = (
            "npub1039j8zfxafe5xtx5qhmjf02rv7upgwgx54kd35e5qehj36egkjuqx9f704"
        )
        session["nsec"] = None
        session["relays"] = TC_RELAYS
        session["libraries"] = TC_LIBRARIES
        ...
        data = {
            "refresh": "shelves",
            "npubValue": "npub1039j8zfxafe5xtx5qhmjf02rv7upgwgx54kd35e5qehj36egkjuqx9f704",
        }
        response = self.client.post(self.url, data, content_type="application/json")
        visible_isbns = []
    ...
    def test_library_post_remove(self):
        ...
        response = self.client.post(
            ...
            {
                "remove_book": "true",
                "book_info": "fe7046323fc3ccc7c6b2748ba58295fc4206a1a3-9780007560776",
            },
        )
    ...
    def test_library_post_moved_to_cr_from_trw(self):
        ...
        response = self.client.post(
            ...
            {
                "moved": "true",
                "book_info": "f76a2d0c13b20a32eeefc4e4f5b393f7b0d6dccc-9780718183899",
                "status": "CR",
            },
        )
        sys.stdout = sys.__stdout__
        ...
        self.assertEqual(
            response.context["progress"]["9780718183899"]["id"],
            sha256("9780718183899".encode()).hexdigest(),
        )
    ...
    def test_library_post_moved_to_hr_from_trw(self):
        ...
        enDt = stDt
        self.assertEqual(
            ["9781444780109", "9781408898956", "0140232699", "9780718183899"], hr_books
        )
        self.assertIn("9780718183899", response.context["progress"].keys())
        self.assertEqual(
            response.context["progress"]["9780718183899"]["id"],
        ...
        self.assertEqual(response.context["progress"]["9780718183899"]["unit"], "pages")
        self.assertEqual(
            response.context["progress"]["9780718183899"]["progress"], "100"
        )
        self.assertEqual(
            response.context["progress"]["9780718183899"]["default"], "320"
        )
    ...
    def test_library_post_move_from_cr_to_hr(self):
        ...
        enDt = stDt
        self.assertEqual(
            ["9781444780109", "9781408898956", "0140232699", "9780007560776"], hr_books
        )
        self.assertIn("9780007560776", response.context["progress"].keys())
        self.assertEqual(
            response.context["progress"]["9780007560776"]["id"],
            sha256("9780007560776".encode()).hexdigest(),
        )
        ...
        self.assertEqual(
            response.context["progress"]["9780007560776"]["progress"], "100"
        )
        self.assertEqual(
            response.context["progress"]["9780007560776"]["default"], "NOT AVAILABLE"
        )
    ...
    def test_library_post_move_from_hr_to_cr(self):
        ...
        self.assertEqual(
            response.context["progress"]["9781444780109"]["id"],
        ...
        self.assertEqual(
            response.context["progress"]["9781444780109"]["default"], "NOT AVAILABLE"
        )
        for book in old_lib["b"]:
    ...
    def test_library_post_update_invalid_dates(self):
        ...
        self.assertEqual(
            "false:End date is before start date.", response.context["noted"]
        )
        ...
        self.assertEqual(
            "false:Start date is required when adding end date.",
            response.context["noted"],
        )
    ...
    def test_library_post_update_invalid_progress(self):
        ...
        self.assertEqual(
            "false:Current progress is greater than max.", response.context["noted"]
        )
    ...
    def test_library_post_update_valid_start(self):
        ...
        # output = capturedOutput.getvalue().strip()
        self.assertEqual(
            response.context["progress"]["9780007560776"]["st"], "2023-01-02"
        )
    ...
    def test_library_post_update_valid_end(self):
        ...
        # output = capturedOutput.getvalue().strip()
        self.assertEqual(
            response.context["progress"]["9781444780109"]["en"], "2023-01-02"
        )
